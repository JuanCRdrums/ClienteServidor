...
def searchValue(value,pred,succ,Rpred,Rsucc,nodeValue):
    poller = zmq.Poller()
    poller.register(pred,zmq.POLLIN)
    poller.register(Rsucc,zmq.POLLIN)
    toSend = {}
    toSend[nodeValue] = value
...
def makeFingerTable(pred,succ,value):
    fingerTable = {}
    # initialize keys:
    for i in range(8):
        fingerTable[str(value + 2**i)] = ''
...
searchValue(200,pred,succ,Rpred,Rsucc,value)
